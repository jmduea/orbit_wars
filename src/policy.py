...
class AttentionPlanetPolicy(nn.Module):
    ...
    def forward(
        ...
        context_query = self.context_query(torch.cat([self_hidden, global_hidden], dim=-1)).unsqueeze(1)
        ...
        attended_context = self.context_norm(context_query + attended_context).squeeze(1)

        expanded_self = self_hidden.unsqueeze(1).expand(-1, self.candidate_count, -1)
        expanded_global = global_hidden.unsqueeze(1).expand(-1, self.candidate_count, -1)
        expanded_context = attended_context.unsqueeze(1).expand(-1, self.candidate_count, -1)
        target_input = torch.cat([expanded_self, expanded_global, expanded_context, attended_candidates], dim=-1)
...
def build_policy(
    *,
    ...
    if normalized_architecture == "attention":
    ...
    raise ValueError(f"Unsupported model architecture '{architecture}'. Expected 'mlp' or 'attention'.")
