...
- JAX layout: rollout `src/jax/rollout/collect.py`, PPO `src/jax/ppo_update.py`, learner sampling `src/jax/action_sampling.py`, shield `src/jax/shield/*` (Python refs in `src/game/shield.py`, `shield_config.py`), train loop `src/jax/train/`, opponents `src/opponents/jax_actions/`; `OPPONENT_FAMILY_*` in `src/opponents/constants.py`; import `src.opponents.pool` only for pool logic. Per-format timing (`*_2p`/`*_4p`) only when `metric_groups.debug`; `average_placement_4p` stays default.
