...
- **Preflight gates:** Gates 2–4 read JAX learning-signal trend from `logs/*_jax.jsonl`; Gate 5 is tournament win proof on checkpoints via `kaggle_environments` (not Docker). Thresholds: `docs/benchmarks/preflight-calibration.json`. Train overrides and evaluation semantics live in `conf/benchmark/gates/*.yaml` (loaded by `src/jax/preflight_gate_loader.py`); do not add tuple tables in `preflight.py`. Primitives: `ow benchmark gate run <name>`, `ow benchmark tournament-proof`; composer: `ow benchmark learn-proof`.
