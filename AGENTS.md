...
- **JAX/Kaggle env parity:** Kaggle reference `kaggle_environments/.../orbit_wars/orbit_wars.py`; JAX `src/jax/env.py`; verify with `make test-kaggle-parity` (`tests/test_jax_env_parity.py`). Training `reset()` uses Kaggle `generate_planets` in `src/game/planet_generation.py` via `jax.pure_callback`; comet spawn/movement uses `src/game/comet_generation.py` at `COMET_SPAWN_STEPS` with per-spawn RNG `orbit_wars-comet-{episode_seed}-{step+1}`; live tournament obs replay via `jax_game_from_observation_fast`. Planet rotation index matches Kaggle `obs.step`. Rollout opponents call `encode_turn(game, cfg.task)` without `feature_history` (learner history is updated in env `_finish_step`); Phase B `opp_batch_cache` must not reuse learner `feature_history`.
...
- **Kaggle bracket ranking:** Competition uses μ/σ episode ratings (margin-independent). Forced qualifier ladder noop → random → nearest_sniper at combined 1.0 before main bracket; incumbent-lineage training skips qualifiers; self-play samples main bracket. Novel configs: pass preflight gates 0–3, then `artifacts=bracket_training` (`conf/artifacts/bracket_training.yaml`) with periodic `qualifier_eval` on held-out seeds; default 500M env-step qualifier budget — exhaust without clear → `weak_config` for calibration evidence. Plan: `docs/plans/2026-06-03-005-feat-kaggle-bracket-ranking-plan.md`; inspect via `ow eval bracket`.
