...
- **JAX/Kaggle env parity:** Kaggle reference `kaggle_environments/.../orbit_wars/orbit_wars.py`; JAX `src/jax/env.py`; verify with `make test-kaggle-parity` (`tests/test_jax_env_parity.py`). Training `reset()` uses Kaggle `generate_planets` in `src/game/planet_generation.py` via `jax.pure_callback`; live tournament obs replay via `jax_game_from_observation_fast`. Planet rotation index matches Kaggle `obs.step`. **Not in JAX:** comet spawn/movement (`COMET_SPAWN_STEPS`). Rollout opponents call `encode_turn(game, cfg.task)` without `feature_history` (learner history is updated in env `_finish_step`); Phase B `opp_batch_cache` must not reuse learner `feature_history`.
