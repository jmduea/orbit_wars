...
- **Planet Flow M1 pipeline:** `ow benchmark shortlist-planet-flow-sweep` → `planet-flow-noop-smoke` → `learn-proof --train-overrides` (window-mean KL guardrails; `training=planet_flow` profile). See `docs/benchmarks/preflight-calibration.md`.
