...
- **JAX/Kaggle env parity:** Kaggle reference `kaggle_environments/.../orbit_wars/orbit_wars.py`; JAX `src/jax/env.py`; verify with `make test-kaggle-parity` (`tests/test_jax_env_parity.py`). Default training uses `task.env_parity_mode=train` (JAX-native reset, no comet spawn callbacks). Set `task=kaggle_parity` or `task.env_parity_mode=kaggle` for Kaggle `generate_planets` / comet `pure_callback` paths; live tournament obs replay via `jax_game_from_observation_fast`. Planet rotation index matches Kaggle `obs.step`. Rollout opponents call `encode_turn(game, cfg.task)` without `feature_history` (learner history is updated in env `_finish_step`); Phase B `opp_batch_cache` must not reuse learner `feature_history`.
