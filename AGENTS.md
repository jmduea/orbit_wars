...
- Learning proof outranks throughput for admission gates; use **one unified training config** for both the learning check and speed extract — do not assume different configs for each. Phase 2 env-parity: implement on `orbit_wars-integration`; greenfield JAX when cherry-pick is blocked (`no_source_commits`) — not main-first; agents run per-pick fast gates only (`make test-kaggle-parity`, `make test-jax-trace-hygiene`), operator runs `gate-admission`/tier-2 after parity stack is green.
- Phase 2 env parity targets **mechanical fidelity** (valid rules-compliant JAX states), not seed/bit-exact replay parity with `kaggle_environments`; maps and comet paths may differ per seed.
...
- **JAX/Kaggle env parity:** Kaggle reference `kaggle_environments/.../orbit_wars/orbit_wars.py`; JAX `src/jax/env.py` (single JIT-safe path — **no** `pure_callback`, **no** `_reference_*` helpers, **no** `src/game/*` imports in the env hot path; **no** parallel Python/callback fallbacks in train/rollout/env hot path). Parity via `src/jax/planet_generation.py` and `src/jax/comet_generation.py` validated in tests against `src/game/*` from test code only; see `docs/solutions/conventions/jax-no-kaggle-callbacks.md`. Verify with `make test-kaggle-parity`. Live tournament obs replay via `jax_game_from_observation_fast`. Planet rotation index matches Kaggle `obs.step`. Rollout opponents call `encode_turn(game, cfg.task)` without `feature_history` (learner history is updated in env `_finish_step`); Phase B `opp_batch_cache` must not reuse learner `feature_history`.
