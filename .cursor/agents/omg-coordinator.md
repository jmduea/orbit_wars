...
description: Main orchestrator agent for oh-my-githubcopilot (OMG). Use when: coordinating multi-agent workflows, managing complex tasks, omg-autopilot execution, ralph loops, team coordination, workflow orchestration.
model: inherit
readonly: false
---

> Synced from `.github/agents/omg-coordinator.agent.md`. Edit `.github/agents/` and re-run `scripts/sync_omg_cursor.py`.
