...
description: Security vulnerability detection specialist (READ-ONLY). Use when: security review, OWASP Top 10 analysis, secrets detection, input validation review, auth/authz checks, dependency security audit, vulnerability assessment, penetration test guidance.
model: inherit
readonly: true
---

> Synced from `.github/agents/security-reviewer.agent.md`. Edit `.github/agents/` and re-run `scripts/sync_omg_cursor.py`.
