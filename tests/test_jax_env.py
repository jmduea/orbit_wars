...
def test_jax_batched_reset_and_step_shapes():
    ...
    assert batches.self_features.shape == (3, MAX_PLANETS, self_feature_dim())
    ...
    assert batches.global_features.shape == (3, MAX_PLANETS, global_feature_dim())
    assert batches.candidate_mask.shape == (3, MAX_PLANETS, cfg.candidate_count)
...
def test_jax_candidates_are_sorted_by_distance_before_id_tiebreaker():
    ...
    planets = JaxPlanetState(
        ...
        ships=jnp.ones((MAX_PLANETS,), dtype=jnp.float32) * 10.0,
...
def test_jax_owner_relative_feature_shapes_and_values_for_four_players():
    ...
    active = jnp.zeros((MAX_PLANETS,), dtype=bool)
    active = active.at[:8].set(
        jnp.array([True, True, True, True, True, False, False, False])
    )

    x = jnp.zeros((MAX_PLANETS,), dtype=jnp.float32)
    x = x.at[:8].set(
        jnp.array([10.0, 12.0, 14.0, 16.0, 18.0, 0.0, 0.0, 0.0], dtype=jnp.float32)
    )

    y = jnp.zeros((MAX_PLANETS,), dtype=jnp.float32)
    y = y.at[:8].set(
        jnp.array([10.0, 10.0, 10.0, 10.0, 10.0, 0.0, 0.0, 0.0], dtype=jnp.float32)
    )

    ships = jnp.zeros((MAX_PLANETS,), dtype=jnp.float32)
    ships = ships.at[:8].set(
        jnp.array([20.0, 30.0, 40.0, 50.0, 60.0, 0.0, 0.0, 0.0], dtype=jnp.float32)
    )
    planets = JaxPlanetState(
        id=planet_ids,
        owner=owner,
    ...
    assert encoded.self_features.shape == (MAX_PLANETS, self_feature_dim())
    assert encoded.candidate_features.shape == (
        MAX_PLANETS,
        cfg.candidate_count,
        candidate_feature_dim(),
    )
    assert encoded.global_features.shape == (MAX_PLANETS, global_feature_dim())
    np.testing.assert_allclose(
        np.asarray(encoded.self_features[0, 11:15]), np.full(4, 1.0 / MAX_PLANETS)
    )
    np.testing.assert_allclose(
    ...
    np.testing.assert_allclose(
        np.asarray(encoded.candidate_features[0, 1, 14:18]),
        np.array([0.0, 1.0, 0.0, 0.0]),
    )
    np.testing.assert_allclose(
        np.asarray(encoded.global_features[0, 8:12]), np.full(4, 1.0 / MAX_PLANETS)
    )
    np.testing.assert_allclose(
...
def test_jax_history_shapes_use_configured_window():
    ...
    assert batch.global_features.shape == (MAX_PLANETS, global_feature_dim(cfg))
