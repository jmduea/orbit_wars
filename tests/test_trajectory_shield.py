...
def test_python_candidate_mask_blocks_selected_target_intercepts_but_allows_non_friendly() -> None:
    planets = [
    ...
    assert not bool(selected_target_batch.candidate_mask[0, selected_target_slot])
    assert bool(non_friendly_batch.candidate_mask[0, non_friendly_target_slot])


def test_mask_policy_output_for_shield_forces_pointer_followups_to_noop() -> None:
    ...
    assert (later_steps[:, 1:] < -1.0e30).all()
    ...
    real_ship_logits = np.asarray(masked.ship_logits[0, 0, 1, :])
    assert real_ship_logits[0] < -1.0e30
